...
//		public Material PersonMaterial;
		...
		public GameObject SpeedTrapObjects;
		public GameObject CheckPointObjects;
        #endregion

        #region int
        public int totalLaps = 3;
        public int totalRacers = 6; //The total number of racers (player included)
        ...
        #region bool
		public bool startCountdown;//uday access specifier changed here
        ...
        void SpawnRacers()
        {
			//ashwini to make first levels easy 
			if (MenuManager.CurrentLevel <= 3) {
				for (int i = 0; i < opponentCars.Count; i++) {
					print ("workinggggggg");
					opponentCars [i].GetComponent<Car_Controller> ().topSpeed = 80;
				}
			} else
			{
				for (int i = 0; i < opponentCars.Count; i++) {
					print ("workinggggggg");
					opponentCars [i].GetComponent<Car_Controller> ().topSpeed = 150;
				}
			}
            if (!playerCar)
            {
            ...
            {
				playerStartRank = Random.Range(5,Mathf.Clamp(9,5,totalRacers));
            }
            ...
            for (int i = 0; i < totalRacers; i++)
            {
                ...
                else if (spawnpoints[i] == spawnpoints[playerStartRank - 1] && playerCar)
                {
					...
					Debug.Log ("spawn "+playerCar.name);
                    GameObject player = (GameObject)Instantiate(playerCar, spawnPos.position, spawnPos.rotation);

					//Motorbike_Controller Player_Bike = player.GetComponent<Motorbike_Controller> ();


					//Player_Bike.topSpeed = 150;//MenuManager.mee.BikeMaxSpeed[PlayerPrefs.GetInt ("MVehicleIndex")];
					//PersonMaterial.mainTexture =MenuManager.mee.PersonMaterial[PlayerPrefs.GetInt ("MVehicleIndex")];
					//BikeMaterial.mainTexture =MenuManager.mee.BikeMaterial[PlayerPrefs.GetInt ("MVehicleIndex")];
        ...
        public IEnumerator Countdown(float delay)
        {
			...
			//mainplayer.GetComponent<Motorbike_Controller> ().MyANim.SetTrigger ("startEngine");
...
//            CameraManager.instance.ActivatePlayerCamera();
            ...
            if (
				...
				SoundManager.instance.musicStart == SoundManager.MusicStart.BeforeCountdown)
...
//                    SoundManager.instance.PlayDefaultSound(SoundManager.instance.defaultSounds.startRaceSound);
        ...
        public void EndRace(int rank)
        {
			...
			StartCoroutine(EndRaceRoutine(rank));

            raceCompleted = true;
			Debug.Log ("end race "+rank);
            CalculateRaceRewards(rank);

            if (ReplayManager.instance)
                ReplayManager.instance.StopRecording();

			Debug.Log (MenuManager.CurrentLevel+" unlocked level before "+PlayerPrefs.GetInt (("UnlockedLevels")));


			if (rank <= 3) {
				
				if(MenuManager.CurrentLevel==PlayerPrefs.GetInt (("UnlockedLevels"))){
				...
				RaceWon = false;
				MenuManager.CurrentLevel -= 1;
				//....................................................................................................


			}

			Debug.Log (" unlocked level after "+PlayerPrefs.GetInt (("UnlockedLevels")));

        }
		public static bool StopAnims = false;
		IEnumerator EndRaceRoutine(int rank)
        {
...
//				RaceUI.instance.player.GetComponent<Car_Controller> ().enabled = false;

			} else {
				RaceUI.instance.SetFinishedText ("RACE COMPLETED");//uday




				if(RaceUI.instance.failRacePanel)
				RaceUI.instance.failRacePanel.SetActive (true);//uday

//				AdManager.instance.RunActions (AdManager.PageType.LF,CurrentTrackDetails.instance.currentLevel);

				if(RaceUI.instance.raceCompletePanel)
					RaceUI.instance.raceCompletePanel.transform.position=new Vector3(0,1000,0);//uday
				print("FAILED+++++++++++++++++++++++++++++");

			}
			int randomNumm = Random.Range (0,3);
//			if (rank <= 3) {
//				
//				if (randomNumm == 0) {
//					RaceUI.instance.ChangeAnim ("actA", 1);
//				}
//
//				if (randomNumm == 1) {
//					RaceUI.instance.ChangeAnim ("actB", 1.5f);
//				}
//				if (randomNumm == 2) {
//					RaceUI.instance.ChangeAnim ("actC", 1.5f);
//				}
...
//			}
			...
			//Debug.Log ("our position "+mainplayer.rank);
        ...
        void CalculateRaceRewards(int pos)
        {
			...
			RaceUI.instance.SetRewardText(MenuManager.CurrentLevelReward,pos);
            ...
            if (raceRewards.Count >= pos)
            {
                ...
                if (raceRewards[pos - 1].currency > 0)
                {
					...
					//MenuManager.menuTracks [MenuManager.CurrentLevel].price;
					//RaceUI.instance.SetRewardText(raceRewards[pos - 1].currency.ToString("N0"), "", "");
					RaceUI.instance.SetRewardText(MenuManager.CurrentLevelReward.ToString("N0"), "", "");
        ...
        public void KnockoutRacer(Statistics racer)
        {
            ...
            if (racer.tag == "Player")
            {
                ...
                //RaceUI Fail Race Panel Config
                 title = (_raceType == RaceType.Elimination) ? "ELIMINATED" : (_raceType == RaceType.LapKnockout) ? "KNOCKED OUT" : "TIMED OUT";
                 reason = (_raceType == RaceType.Elimination) ? "You were eliminated from the race." : (_raceType == RaceType.LapKnockout) ? "You were knocked out of the race." : "You ran out of time.";


				string keyword = (_raceType == RaceType.Elimination) ? " eliminated." : (_raceType == RaceType.LapKnockout) ? " knocked out." : " timed out.";

				RaceUI.instance.ShowRaceInfo("you" + keyword, 2.0f, Color.white);

				StopAnims = true;
				//racer.StopBike();

//				RaceUI.instance.ChangeAnim ("OneLegControlAnim", 1);
				RaceUI.instance.HintFailImage.transform.parent.gameObject.SetActive (true);
				Invoke ("AfterDelayFail",2);
				PlayerCamera.mee.ChangeCameraToGreyScaleEnable ();

				iTween.ValueTo (gameObject,iTween.Hash("from",0.1,"to",1f,"time",.5f,"onupdate","ChangeVal"));
		...
		void ChangeVal(float newValue){
			//apply the value of newValue:

			RaceUI.instance.HintFailImage.fillAmount = newValue;
			if(newValue==1){
				
				iTween.ValueTo (gameObject,iTween.Hash("from",0.92,"to",0f,"delay",1,"time",.5f,"onupdate","ChangeVal"));

			}
			if(newValue==0){
				RaceUI.instance.HintFailImage.transform.parent.gameObject.SetActive (false);
				PlayerCamera.mee.ChangeCameraToGreyScaleDisable ();
        ...
        //Format a float to a time string for main player
		...
		//Format a float time to the AIcars

		public string AI_FormatTime(float time)
		{
			int minutes = (int)Mathf.Floor(time / 60);
			int seconds = (int)time % 60;
			int milliseconds = (int)(time * 100) % 100;
        ...
        IEnumerator Respawn(Transform racer, Transform node, float ignoreCollisionTime)
        {
            ...
            racer.position = new Vector3(node.position.x, node.position.y + 10, node.position.z);
        ...
        private int SetValue(int val, int otherVal)
        {
			...
			Debug.Log (otherVal+" total "+myVal+" : "+val);
            return myVal;
        }
