...
namespace RGSK
{
    public class PlayerControl : MonoBehaviour
    {
        ...
        public enum MobileSteerType { TiltToSteer, TouchSteer,SteerToSteer }
        ...
        void Awake()
        {
			...
			SteerValText = GameObject.Find ("Steerval");
        }
        ...
        void DesktopControl()
        {
			//Debug.LogError ("Mobile controls type::"+mobileSteerType);
...
//				Debug.LogError ("Ketyboard steer value::"+steer);
                    //Send inputs
		...
		public	float steerAmount = 0;


        void MobileControl()
        {

//			SteerValText.GetComponent<Text>().text= "Steer Control::" + mobileSteerType;
            if (mobileSteerType == MobileSteerType.TiltToSteer)
            {
                //steer according to the device tilt amount
                steer = Input.acceleration.x;
				Debug.LogError ("Mohith Debug for TILT STEER::"+steer);
            }
			 if(mobileSteerType==MobileSteerType.TouchSteer)
            {
                ...
                if (InputManager.instance.mobileInput.steerRight != null && InputManager.instance.mobileInput.steerLeft != null)
                {
					...
					Debug.LogError ("Mohith Debug for Touch STEER::"+steer);

                }
//				SteerValText.GetComponent<Text>().text="Steer Val::"+steer;

            }
			if (mobileSteerType == MobileSteerType.SteerToSteer) 
			{

				steerAmount =SteeringWheel.mee.SteerAngleVal;

//				SteerValText.GetComponent<Text> ().text = SteeringWheel.mee.SteerAngleVal+" Steeramount::" + steerAmount;
				if (steerAmount != 0) {
					steer = steerAmount;
				}
