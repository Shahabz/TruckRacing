...
public class RGSK_Editor : Editor
{
    ...
    public static void CreateRaceCameras()
    {
      ...
      //  playerCam.AddComponent<GUILayer>();
     ...
     //   cinematicCam.AddComponent<GUILayer>();
    ...
    /*[MenuItem("Window/RacingGameStarterKit/Vehicle Configuration/Misc/Fix Wheel Rotation")]
    public static void FixWheelRotation()
    {
        GameObject wheel = Selection.activeGameObject;

        if (wheel == null) return;

        GameObject newWheel = new GameObject(wheel.name + " Fix");
        newWheel.transform.position = wheel.transform.position;
        newWheel.transform.SetParent(wheel.transform.parent,false);
        wheel.transform.SetParent(newWheel.transform,false);
    }
    */
