...
namespace RGSK
{
    public class Motorbike_Controller : MonoBehaviour
    {
        ...
        void Ride()
        {
			...
			if (!IsAi) {
				//Debug.Log (currentSpeed+" : "+rigid.velocity.normalized);
			
				//Debug.Log (velocityDir);
				//Debug.Log (maxLeanAngle+" : "+currentSpeed);

				maxLeanAngle = Mathf.Clamp ((currentSpeed * 0.25f), 1, 28);


				if (velocityDir.z >= 1) {
					MovingDirection = 1;
				} else if (velocityDir.z <= -1) {
					MovingDirection = -1;
				...
				if(MovingDirection==-1 && currentSpeed!=0){
					//Debug.Log ("--- "+currentSpeed);
					ChangeAiAnim("BackWardAnim",1);
				}

				else if(targetAngle.z<=-0.1f && currentSpeed!=0){
					//Debug.Log ("right ");
					ChangeAiAnim("leanrightAnim",0.3f);

				}

				//Player_biker.frontWheelCollider.steerAngle
				else if(targetAngle.z>=0.1f && currentSpeed!=0 ){
					//Debug.Log ("right ");

					ChangeAiAnim("leanleftAnim",0.3f);

				}
				else if(currentSpeed<=2){

					if (RaceManager.instance._raceState == RaceManager.RaceState.Racing) {
						ChangeAiAnim ("OneLegControlAnim", 1);
					}

				}
				else if(currentSpeed>=3 && currentSpeed<=4){
					ChangeAiAnim("startfromzeroAnim",1);

				}

				else if(currentSpeed>=5){
					ChangeAiAnim("driveloopanim",0.5f);
		...
		public void ChangeAiAnim(string AnimToPlay,float SpeedAnim){
			if (MyANim != null) {
				
				if (AnimToPlay != PlayingAnim) {
					PlayingAnim = AnimToPlay;
					MyANim.SetTrigger (PlayingAnim);
				}
        ...
        void LeanChassis()
        {
            ...
            lean = Mathf.Clamp(Mathf.Lerp(lean, (transform.InverseTransformDirection(rigid.angularVelocity).y * normalAngle) * leanAmount, Time.deltaTime * leanDamping), -maxLeanAngle, maxLeanAngle);
             ...
             targetAngle = Quaternion.Euler(0, chassis.transform.localEulerAngles.y, lean);
...
//			}
        ...
        void Nitro()
        {
            if (usingNitro && nitroCapacity > 0f && motorInput > 0)
            {
				...
				if (!IsAi) {
				//	Debug.Log ("in nitro");
					RaceUI.instance.MotionObject.blurAmount =0.5f;
...
//						RaceUI.instance.MotionObject.Aireffect.SetActive (true);//uday
					}
...
//						RaceUI.instance.MotionObject.Aireffect.SetActive (false);//uday
					} else {
						RaceUI.instance.MotionObject.blurAmount = 0f;
//						RaceUI.instance.MotionObject.Aireffect.SetActive (false);//udya
					}
        ...
        {
			ExactSpeed = (_speedUnit == SpeedUnit.MPH) ? rigid.velocity.magnitude * 2.237f : rigid.velocity.magnitude * 3.6f;;
